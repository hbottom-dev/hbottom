...
static void OpenBSDMachine_scanMemoryInfo(OpenBSDMachine* this) {
    ...
    * Modified through hbottom developers applying GPL-2
    */
...
static void kernelCPUTimesTohbottom(const u_int64_t* times, CPUData* cpu) {
...
static void OpenBSDMachine_scanCPUTime(OpenBSDMachine* this) {
   ...
   for (unsigned int i = 0; i < super->existingCPUs; i++) {
      ...
      kernelCPUTimesTohbottom(kernelTimes, cpu);
   ...
   kernelCPUTimesTohbottom(avg, &this->cpuData[0]);
