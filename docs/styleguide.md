...
The only exceptions here are `hbottom.c` and `pcp-hbottom.c`, which contain the main entrance points into the code.
...
We use the GPLv2+ as a shorthand indication that we release `hbottom` under the GNU Public license version 2 but are totally fine with users opting to apply the "any later version" clause.
...
While they usually provide quite a performance gain or make your life easier, it is beneficial if `hbottom` runs on rather ancient systems.
...
The primary user documentation should be the man file which you can find in `hbottom.1.in`.
