...
static void CRT_handleSIGTERM(int sgn) {
   ...
   snprintf(err_buf, sizeof(err_buf),
           "A signal %d (%s) was received, exiting without persisting settings to hbottomrc.\n",
           sgn, signal_str);
...
static int createStderrCacheFile(void) {
   ...
   return memfd_create("hbottom.stderr-redirect", 0);
#elif defined(O_TMPFILE)
   return open("/tmp", O_TMPFILE | O_CREAT | O_EXCL | O_RDWR, S_IRUSR | S_IWUSR);
#else
   char tmpName[] = "hbottom.stderr-redirectXXXXXX";
...
void CRT_handleSIGSEGV(int signal) {
   ...
   snprintf(err_buf, sizeof(err_buf), "\n\n"
      ...
      "============================\n"
      "Please check at https://hbottom.dev/issues whether this issue has already been reported.\n"
