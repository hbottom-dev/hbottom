...
static void FreeBSDMachine_scanMemoryInfo(Machine* super) {
   ...
   //  hbottom_used (unavail to anybody)   = active + wired + inactive - buffer
   //  hbottom_cache (for cache meter)     = buffer + cache
   //  hbottom_user_free (avail to procs)  = buffer + cache + free
   //  hbottom_buffers (disk write buffer) = 0 (not applicable to FreeBSD)
   //
   ...
   //
   // hbottom_used   = active + (wired - arc)
   // hbottom_cache  = buffers + cache + arc
   u_long totalMem;
