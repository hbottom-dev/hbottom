...
static void LinuxMachine_scanHugePages(LinuxMachine* this) {
   ...
   for (unsigned i = 0; i < hbottom_HUGEPAGE_COUNT; i++) {
   ...
   while ((entry = readdir(dir)) != NULL) {
      ...
      int shift = ffsl(hugePageSize) - 1 - (hbottom_HUGEPAGE_BASE_SHIFT - 10);
      assert(shift >= 0 && shift < hbottom_HUGEPAGE_COUNT);
