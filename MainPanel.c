...
static HandlerResult MainPanel_eventHandler(Panel* super, int ch) {
   ...
   if (EVENT_IS_HEADER_CLICK(ch)) {
      ...
      reaction |= hbottom_RECALCULATE | hbottom_REDRAW_BAR | hbottom_UPDATE_PANELHDR | hbottom_SAVE_SETTINGS;
      result = HANDLED;
      ...
      if (filterChanged) {
         ...
         reaction = hbottom_REFRESH | hbottom_REDRAW_BAR;
      }
      if (this->inc->found) {
         reaction |= Action_follow(this->state);
         reaction |= hbottom_KEEP_FOLLOWING;
      }
      result = HANDLED;
   } else if (ch == 27) {
   ...
   if ((reaction & hbottom_REDRAW_BAR) == hbottom_REDRAW_BAR) {
      MainPanel_updateLabels(this, settings->ss->treeView, host->activeTable->incFilter);
   }
   if ((reaction & hbottom_RESIZE) == hbottom_RESIZE) {
      result |= RESIZE;
   }
   if ((reaction & hbottom_UPDATE_PANELHDR) == hbottom_UPDATE_PANELHDR) {
      result |= REDRAW;
   }
   if ((reaction & hbottom_REFRESH) == hbottom_REFRESH) {
      result |= REFRESH;
   }
   if ((reaction & hbottom_RECALCULATE) == hbottom_RECALCULATE) {
      result |= RESCAN;
   }
   if ((reaction & hbottom_SAVE_SETTINGS) == hbottom_SAVE_SETTINGS) {
      host->settings->changed = true;
   }
   if ((reaction & hbottom_QUIT) == hbottom_QUIT) {
      return BREAK_LOOP;
   }
   if ((reaction & hbottom_KEEP_FOLLOWING) != hbottom_KEEP_FOLLOWING) {
...
MainPanel* MainPanel_new(void) {
   ...
   this->keys = xCalloc(KEY_MAX, sizeof(hbottom_Action));
