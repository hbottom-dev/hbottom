...
bool TraceScreen_forkTracer(TraceScreen* this) {
   ...
   if (child == 0) {
      ...
      #if defined(hbottom_FREEBSD) || defined(hbottom_OPENBSD) || defined(hbottom_NETBSD) || defined(hbottom_DRAGONFLYBSD) || defined(hbottom_SOLARIS)
      ...
      #elif defined(hbottom_LINUX)
         execlp("strace", "strace", "-T", "-tt", "-s", "512", "-p", buffer, (void*)NULL);

         // Should never reach here, unless execlp fails ...
         const char* message = "Could not execute 'strace'. Please make sure it is available in your $PATH.";
         (void)! write(STDERR_FILENO, message, strlen(message));
      #else // hbottom_DARWIN, hbottom_PCP == hbottom_UNSUPPORTED
