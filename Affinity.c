...
#ifdef __linux__
#define hbottom_HWLOC_CPUBIND_FLAG HWLOC_CPUBIND_THREAD
#else
#define hbottom_HWLOC_CPUBIND_FLAG HWLOC_CPUBIND_PROCESS
#endif
...
static Affinity* Affinity_get(const Process* p, Machine* host) {
   ...
   bool ok = (hwloc_get_proc_cpubind(host->topology, Process_getPid(p), cpuset, hbottom_HWLOC_CPUBIND_FLAG) == 0);
...
static bool Affinity_set(Process* p, Arg arg) {
   ...
   bool ok = (hwloc_set_proc_cpubind(this->host->topology, Process_getPid(p), cpuset, hbottom_HWLOC_CPUBIND_FLAG) == 0);
   hwloc_bitmap_free(cpuset);
   return ok;
}
