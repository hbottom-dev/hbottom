...
int Settings_write(const Settings* this, bool onCrash) {
   ...
   if (!onCrash) {
      of(fp, "# Beware! This file is rewritten by hbottom when settings are changed in the interface.\n");
      of(fp, "# The parser is also very primitive, and not human-friendly.\n");
   }
   printSettingString("hbottom_version", VERSION);
   ...
   // Legacy compatibility with older versions of hbottom
...
Settings* Settings_new(const Machine* host, Hashtable* dynamicMeters, Hashtable* dynamicColumns, Hashtable* dynamicScreens) {
   ...
   const char* rcfile = getenv("hbottomRC");
   if (rcfile) {
      ...
      char* configDir = NULL;
      char* hbottomDir = NULL;
      if (xdgConfigHome && xdgConfigHome[0] == '/') {
         this->initialFilename = String_cat(xdgConfigHome, "/hbottom/hbottomrc");
         configDir = xStrdup(xdgConfigHome);
         hbottomDir = String_cat(xdgConfigHome, "/hbottom");
      } else {
         this->initialFilename = String_cat(home, CONFIGDIR "/hbottom/hbottomrc");
         configDir = String_cat(home, CONFIGDIR);
         hbottomDir = String_cat(home, CONFIGDIR "/hbottom");
      }
      (void) mkdir(configDir, 0700);
      (void) mkdir(hbottomDir, 0700);
      free(hbottomDir);
      free(configDir);

      legacyDotfile = String_cat(home, "/.hbottomrc");
   }
   ...
   if (!ok) {
      ...
      ok = Settings_read(this, SYSCONFDIR "/hbottomrc", host, /*checkWritability*/false);
   }
   if (!ok) {
      Settings_defaultMeters(this, host);
      Settings_defaultScreens(this);
