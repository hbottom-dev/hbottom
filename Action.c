...
hbottom_Reaction Action_setSortKey(Settings* settings, ProcessField sortKey) {
   ...
   return hbottom_REFRESH | hbottom_SAVE_SETTINGS | hbottom_UPDATE_PANELHDR | hbottom_KEEP_FOLLOWING;
}
...
static hbottom_Reaction actionSetSortColumn(State* st) {
   hbottom_Reaction reaction = hbottom_OK;
   ...
   return reaction | hbottom_REFRESH | hbottom_REDRAW_BAR | hbottom_UPDATE_PANELHDR;
}

static hbottom_Reaction actionSortByPID(State* st) {
   return Action_setSortKey(st->host->settings, PID);
}

static hbottom_Reaction actionSortByMemory(State* st) {
   return Action_setSortKey(st->host->settings, PERCENT_MEM);
}

static hbottom_Reaction actionSortByCPU(State* st) {
   return Action_setSortKey(st->host->settings, PERCENT_CPU);
}

static hbottom_Reaction actionSortByTime(State* st) {
   return Action_setSortKey(st->host->settings, TIME);
}

static hbottom_Reaction actionToggleKernelThreads(State* st) {
   ...
   return hbottom_RECALCULATE | hbottom_SAVE_SETTINGS | hbottom_KEEP_FOLLOWING;
}

static hbottom_Reaction actionToggleUserlandThreads(State* st) {
   Settings* settings = st->host->settings;
...
static hbottom_Reaction actionToggleRunningInContainer(State* st) {
   Settings* settings = st->host->settings;
...
static hbottom_Reaction actionToggleProgramPath(State* st) {
   Settings* settings = st->host->settings;
   settings->showProgramPath = !settings->showProgramPath;
   settings->lastUpdate++;

   return hbottom_REFRESH | hbottom_SAVE_SETTINGS | hbottom_KEEP_FOLLOWING;
}

static hbottom_Reaction actionToggleMergedCommand(State* st) {
   Settings* settings = st->host->settings;
   settings->showMergedCommand = !settings->showMergedCommand;
   settings->lastUpdate++;

   return hbottom_REFRESH | hbottom_SAVE_SETTINGS | hbottom_KEEP_FOLLOWING | hbottom_UPDATE_PANELHDR;
}

static hbottom_Reaction actionToggleTreeView(State* st) {
   Machine* host = st->host;
   ...
   return hbottom_REFRESH | hbottom_SAVE_SETTINGS | hbottom_KEEP_FOLLOWING | hbottom_REDRAW_BAR | hbottom_UPDATE_PANELHDR;
}

static hbottom_Reaction actionToggleHideMeters(State* st) {
   st->hideMeters = !st->hideMeters;
   return hbottom_RESIZE | hbottom_KEEP_FOLLOWING;
}

static hbottom_Reaction actionExpandOrCollapseAllBranches(State* st) {
   Machine* host = st->host;
   ScreenSettings* ss = host->settings->ss;
   if (!ss->treeView) {
      return hbottom_OK;
   ...
   if (ss->allBranchesCollapsed)
      ...
      Table_expandTree(host->activeTable);
   return hbottom_REFRESH | hbottom_SAVE_SETTINGS;
}

static hbottom_Reaction actionIncFilter(State* st) {
   Machine* host = st->host;
   IncSet* inc = (st->mainPanel)->inc;
   IncSet_activate(inc, INC_FILTER, (Panel*)st->mainPanel);
   host->activeTable->incFilter = IncSet_filter(inc);
   return hbottom_REFRESH | hbottom_KEEP_FOLLOWING;
}

static hbottom_Reaction actionIncSearch(State* st) {
   IncSet_reset(st->mainPanel->inc, INC_SEARCH);
   IncSet_activate(st->mainPanel->inc, INC_SEARCH, (Panel*)st->mainPanel);
   return hbottom_REFRESH | hbottom_KEEP_FOLLOWING;
}

static hbottom_Reaction actionHigherPriority(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_OK;

   bool changed = changePriority(st->mainPanel, -1);
   return changed ? hbottom_REFRESH : hbottom_OK;
}

static hbottom_Reaction actionLowerPriority(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_OK;
...
static hbottom_Reaction actionInvertSortOrder(State* st) {
   Machine* host = st->host;
...
static hbottom_Reaction actionExpandOrCollapse(State* st) {
   if (!st->host->settings->ss->treeView)
      return hbottom_OK;

   bool changed = expandCollapse((Panel*)st->mainPanel);
   return changed ? hbottom_RECALCULATE : hbottom_OK;
}

static hbottom_Reaction actionCollapseIntoParent(State* st) {
   if (!st->host->settings->ss->treeView) {
      return hbottom_OK;
   }
...
static hbottom_Reaction actionExpandCollapseOrSortColumn(State* st) {
...
static hbottom_Reaction actionNextScreen(State* st) {
   Settings* settings = st->host->settings;
   settings->ssIndex++;
   ...
   return hbottom_UPDATE_PANELHDR | hbottom_REFRESH | hbottom_REDRAW_BAR;
}

static hbottom_Reaction actionPrevScreen(State* st) {
   Settings* settings = st->host->settings;
...
static hbottom_Reaction actionQuit(ATTR_UNUSED State* st) {
   return hbottom_QUIT;
}

static hbottom_Reaction actionSetAffinity(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_OK;
   ...
   return hbottom_REFRESH | hbottom_REDRAW_BAR | hbottom_UPDATE_PANELHDR;
#else
   return hbottom_OK;
#endif
}
...
static hbottom_Reaction actionSetSchedPolicy(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_KEEP_FOLLOWING;
   ...
   return hbottom_REFRESH | hbottom_REDRAW_BAR | hbottom_KEEP_FOLLOWING;
}
#endif  /* SCHEDULER_SUPPORT */

static hbottom_Reaction actionKill(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_OK;
...
static hbottom_Reaction actionFilterByUser(State* st) {
...
static hbottom_Reaction actionSetup(State* st) {
   Action_runSetup(st);
   return hbottom_REFRESH | hbottom_REDRAW_BAR | hbottom_UPDATE_PANELHDR | hbottom_RESIZE;
}

static hbottom_Reaction actionLsof(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_OK;
   ...
   clear();
   CRT_enableDelay();
   return hbottom_REFRESH | hbottom_REDRAW_BAR;
}

static hbottom_Reaction actionShowLocks(State* st) {
...
static hbottom_Reaction actionStrace(State* st) {
   if (!Action_writeableProcess(st))
      return hbottom_OK;
...
static hbottom_Reaction actionTag(State* st) {
...
static hbottom_Reaction actionRedraw(ATTR_UNUSED State* st) {
   clear();
   // hbottom_RECALCULATE here to make Ctrl-L also refresh the data and not only redraw
   return hbottom_RECALCULATE | hbottom_REFRESH | hbottom_REDRAW_BAR;
}

static hbottom_Reaction actionTogglePauseUpdate(State* st) {
...
static hbottom_Reaction actionHelp(State* st) {
   clear();
   ...
   mvaddstr(line++, 0, "hbottom " VERSION " - " COPYRIGHT);
...
#undef addattrstatestr
   ...
   return hbottom_RECALCULATE | hbottom_REDRAW_BAR | hbottom_KEEP_FOLLOWING;
}

static hbottom_Reaction actionUntagAll(State* st) {
   for (int i = 0; i < Panel_size((Panel*)st->mainPanel); i++) {
      Row* row = (Row*) Panel_get((Panel*)st->mainPanel, i);
      row->tag = false;
   }
   return hbottom_REFRESH;
}

static hbottom_Reaction actionTagAllChildren(State* st) {
...
static hbottom_Reaction actionShowEnvScreen(State* st) {
   if (!Action_readableProcess(st))
      return hbottom_OK;
...
static hbottom_Reaction actionShowCommandScreen(State* st) {
   if (!Action_readableProcess(st))
      return hbottom_OK;
