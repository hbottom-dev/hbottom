/*
hbottom - PCPDynamicScreen.c
(C) 2022 Sohaib Mohammed
(C) 2022-2023 hbottom dev team
...
static PCPDynamicColumn* PCPDynamicScreen_lookupMetric(PCPDynamicScreen* screen, const char* name) {
   ...
   xAsprintf(&metricName, "hbottom.screen.%s.%s", screen->super.name, name);
...
void PCPDynamicScreens_init(PCPDynamicScreens* screens, PCPDynamicColumns* columns) {
   ...
   const char* override = getenv("PCP_hbottom_DIR");
   const char* home = getenv("HOME");
   char* path;

   screens->table = Hashtable_new(0, true);

   /* developer paths - PCP_hbottom_DIR=./pcp ./pcp-hbottom */
   if (override) {
   ...
   /* next, search in home directory alongside hbottomrc */
   if (xdgConfigHome)
      path = String_cat(xdgConfigHome, "/hbottom/screens/");
   else if (home)
      path = String_cat(home, CONFIGDIR "/hbottom/screens/");
   else
      path = NULL;
   if (path) {
      PCPDynamicScreen_scanDir(screens, path);
   ...
   path = String_cat(sysconf, "/hbottom/screens/");
   PCPDynamicScreen_scanDir(screens, path);
   free(path);

   /* next, try the readonly system screens directory */
   path = String_cat(share, "/hbottom/screens/");
   PCPDynamicScreen_scanDir(screens, path);
   free(path);
...
/* called when hbottomrc .dynamic line is parsed for a dynamic screen */
