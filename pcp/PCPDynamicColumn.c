/*
hbottom - PCPDynamicColumn.c
(C) 2021-2023 Sohaib Mohammed
(C) 2021-2023 hbottom dev team
...
static bool PCPDynamicColumn_addMetric(PCPDynamicColumns* columns, PCPDynamicColumn* column) {
   ...
   xAsprintf(&metricName, "hbottom.column.%s", column->super.name);
...
// Ensure a valid name for use in a PCP metric name and in hbottomrc
...
void PCPDynamicColumns_init(PCPDynamicColumns* columns) {
   ...
   if (xdgConfigHome)
      path = String_cat(xdgConfigHome, "/hbottom/columns/");
   else if (home)
      path = String_cat(home, CONFIGDIR "/hbottom/columns/");
   else
      path = NULL;
   if (path) {
      PCPDynamicColumn_scanDir(columns, path);
   ...
   path = String_cat(sysconf, "/hbottom/columns/");
   PCPDynamicColumn_scanDir(columns, path);
   free(path);

   /* next, try the readonly system columns directory */
   path = String_cat(share, "/hbottom/columns/");
   PCPDynamicColumn_scanDir(columns, path);
   free(path);
}
