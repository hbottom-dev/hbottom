...
static PCPDynamicMetric* PCPDynamicMeter_lookupMetric(PCPDynamicMeters* meters, PCPDynamicMeter* meter, const char* name) {
   ...
   xAsprintf(&metricName, "hbottom.meter.%s.%s", meter->super.name, name);
...
void PCPDynamicMeters_init(PCPDynamicMeters* meters) {
   ...
   if (xdgConfigHome)
      path = String_cat(xdgConfigHome, "/hbottom/meters/");
   else if (home)
      path = String_cat(home, CONFIGDIR "/hbottom/meters/");
   else
      path = NULL;
   if (path) {
      PCPDynamicMeter_scanDir(meters, path);
   ...
   path = String_cat(sysconf, "/hbottom/meters/");
   PCPDynamicMeter_scanDir(meters, path);
   free(path);

   /* next, try the readonly system meters directory */
   path = String_cat(share, "/hbottom/meters/");
   PCPDynamicMeter_scanDir(meters, path);
   free(path);
}
