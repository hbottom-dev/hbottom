...
DisplayOptionsPanel* DisplayOptionsPanel_new(Settings* settings, ScreenManager* scr) {
   ...
   Panel_add(super, (Object*) CheckItem_newByRef("- Tree view is always sorted by PID (hbottom 2 behavior)", &(settings->ss->treeViewAlwaysByPID)));
   ...
   #if defined(hbottom_LINUX)
                                                 "Also show CPU temperature (requires libsensors)",
   #elif defined(hbottom_FREEBSD)
